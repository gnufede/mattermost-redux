...
function pluginStatuses(state = {}, action) {
    ...
    // RECEIVED_PLUGIN isn't a plugin status, but use it to pre-populate.
    case AdminTypes.RECEIVED_PLUGIN: {
    ...
    case AdminTypes.ACTIVATE_PLUGIN_FAILURE: {
        const pluginId = action.data;
        if (!state[pluginId]) {
            return state;
        }
        ...
        return {
            ...
            [pluginId]: {
                ...state[pluginId],
                state: PluginState.PLUGIN_STATE_FAILED_TO_STAY_RUNNING,
            },
        };
    }
